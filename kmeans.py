...
def euclidean_distance(point1, point2):
    ...
    """
    sum = 0  # Running sum
    for i in range(len(point1)):  # Loop through all the points
        # Adding the distance between each cordinant squared
        sum += (point1[i] - point2[i]) ** 2
    # Taking the square root of total sum
    euc_distance = (sum) ** (1 / 2)
    return euc_distance  # Returning the Euclidian distance
...
def get_closest_centroid(point, centroids_dict):
    ...
    Example:
        ...
        Output:
            centroid1
    """
    short_dist = 0  # The current shortest distance 0 is placeholder
    closest_cent = "temp"  # current closest centroid, temp is placeholder
    # Loop through all the keys and values in centroids_dict
    for (key, value) in centroids_dict.items():
        # Using function euclidean_distance to calculate the distance
        # between the point and value in centroids_dict
        dist = euclidean_distance(point, value)
        # If this is the first value we looked at, or if it is a shorter
        # distacne
        if short_dist == 0 or dist < short_dist:
            # Update the shortest distance and which key it is
            short_dist = dist
            closest_cent = key
    return closest_cent  # Return the key with the closest centroid
...
def update_assignment(list_of_points, centroids_dict):
    ...
    for point in list_of_points:
        # Finding the closest centroid to the given point
        close_cent = get_closest_centroid(point, centroids_dict)
        # If this closest centroid is not in the new dictionary
        if close_cent not in updated_dic:
            # Add a list for the new closest centroid
            updated_dic[close_cent] = []
        # Add the point into the new list in the new dictionary
        updated_dic[close_cent].append(point)
...
def mean_of_points(list_of_points):
    ...
    """
    # Create a list for the means of the points
    mean_of_points = []
    for point in range(len(list_of_points[0])):
        temp_mean = 0
        for index in range(len(list_of_points)):
            temp_mean = temp_mean + list_of_points[index][point]
        temp_mean = temp_mean / len(list_of_points)
        mean_of_points.append(temp_mean)
    return mean_of_points  # Final mean of the points
...
def update_centroids(assignment_dict):
    ...
    """
    # Creating a dictionary to later be returned
    final_dic = {}
    # For loop to run through the keys and vlaues of
    # the inported assignment_dict
    for (key, value) in assignment_dict.items():
        # Updating the new dictionary with the means
        # of the inputed assignment_dict
        final_dic[key] = mean_of_points(value)
    return final_dic  # Returning the new dictionary
